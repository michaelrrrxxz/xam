...
export function useBatchFilters(batch: Ref<Batch[]>) {
  ...
  const filteredBatch = computed<Batch[]>(() => {
    ...
    if (yearFilter.value !== 'all') {
      data = data.filter(
        (e) => new Date(e.created_at).getFullYear().toString() === yearFilter.value
      );
    }

    return [...data].sort((a, b) => {
      const valA =
        sortKey.value === 'year' ? new Date(a.created_at).getFullYear() : a[sortKey.value];
      const valB =
        sortKey.value === 'year' ? new Date(b.created_at).getFullYear() : b[sortKey.value];
